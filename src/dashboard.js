...
import CropModal from "./components/Homebrew/CropModal.jsx";
...
class Dash extends Component {
  ...
  componentWillMount() {
    if (localStorage.getItem("draft_image") !== "") {
      console.log(this.props);
      // let data = this.props.location.state.data.map((obj, i) => {
      //   return {
      //     cutImage:obj.
      //   }
      // })
      this.setState({ imageUrl: localStorage.getItem("draft_image") });
      this.props.updateBackground(localStorage.getItem("draft_image"));
      this.props.addItem(this.props.location.state.data);
      this.props.updateShow({ showData: this.props.location.state.data });
      localStorage.setItem("draft_image", "");
    }
  }
  ...
  async dragStart(e) {
    ...
    enableDrag = true;
    canvases[currentIndex].initialX = await (e.clientX -
      canvases[currentIndex].xOffset);
    canvases[currentIndex].initialY = await (e.clientY -
      canvases[currentIndex].yOffset);
  }

  async dragEnd(e) {
    // console.log(`drag_End[ ${currentIndex} ]`);
    canvases[currentIndex].initialX = await canvases[currentIndex].currentX;
    canvases[currentIndex].initialY = await canvases[currentIndex].currentY;
    enableDrag = false;
    enableRotate = false;
    enableResize = false;
  }
  ...
  async handleEvents(e) {
    if (enableDrag && !enableRotate && !enableResize) {
      ...
      canvases[currentIndex].currentX = await (e.clientX -
        canvases[currentIndex].initialX);
      canvases[currentIndex].currentY = await (e.clientY -
        canvases[currentIndex].initialY);

      canvases[currentIndex].xOffset = await canvases[currentIndex].currentX;
      canvases[currentIndex].yOffset = await canvases[currentIndex].currentY;
  ...
  handleCanvas() {
    console.log("m in handcanvas");
    const { classes } = this.props;
    console.log(this.props.location.state);
    console.log(this.props.showData, "sjowdata");
    return (
      <div
        id="container"
          ...
          return (
            <div
              ...
              style={{
                ...
                backgroundSize: `${obj.width}vw ${obj.height}vh`,
            ...
            >
              ...
              <Product
                canvasId={canvasid}
                data={obj}
                flipStyle={defaultFlip}
                enable={this.state.confirmationIcons}
              />
            </div>
          );
        })}
  ...
  render() {
          ...
          >
            <Fab
              ...
              onClick={() => {
                if (this.props.tutorialStep !== 5) {
                  console.log(
                    "currentCutout to delete: " + this.props.currentCutout
                  );
                  ...
                  // canvases.splice(
                  //   canvases.indexOf(this.props.currentCutout),
                  //   1
                  // );
                  // console.log(canvases);
                }
    ...
    if (this.props.share) {
      modal = (
        <div>
          <ShareModal imageUrl={this.state.imageUrl} />
        </div>
      );
    } else if (this.props.save) {
      modal = (
        <div>
          <SaveModal imageUrl={this.state.imageUrl} />
        </div>
      );
